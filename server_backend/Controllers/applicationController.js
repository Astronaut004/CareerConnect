...
export const getApplications = async (req, res) => {
    try {
        const result = await pool.query(' SELECT applications.*, jobs.company, jobs.position,  jobs.status  FROM applications INNER JOIN jobs ON applications.job_id = jobs.id');
...
export const withdrawApplication = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Application withdrawn successfully", application: result.rows[0] });
  } catch (error) {
